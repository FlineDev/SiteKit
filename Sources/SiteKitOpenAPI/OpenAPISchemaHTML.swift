...
enum OpenAPISchemaHTML {
   ...
   static func typeLabel(_ node: OpenAPISpec.SchemaNode, context: BuildContext) -> String {
      if let referenceName = node.referenceName {
         let href = OpenAPIHTML.escape(OpenAPIRoutes.schemaPath(context, schemaSlug: OpenAPIRoutes.schemaSlug(referenceName)))
         return "<a class=\"sk-openapi-type-ref\" href=\"\(href)\">\(OpenAPIHTML.escape(referenceName))</a>"
      }
      if let composition = node.composition {
         let members = composition.subschemas.map { self.typeLabel($0, context: context) }.joined(separator: ", ")
         return "<span class=\"sk-openapi-type\" data-composition=\"\(composition.kind.rawValue)\">\(composition.kind.rawValue) (\(members))</span>"
      }
      if node.type == "array" {
         let item = node.items.first.map { self.typeLabel($0, context: context) } ?? "<span class=\"sk-openapi-type\">any</span>"
   ...
   static func propertyTable(_ node: OpenAPISpec.SchemaNode, context: BuildContext) -> String {
      ...
      let rows = node.properties.map { property -> String in
         ...
         return "<tr class=\"sk-openapi-prop\">"
            ...
            + "<td class=\"sk-openapi-prop-type\">\(self.typeLabel(property.schema, context: context))</td>"
   ...
   static func detail(_ node: OpenAPISpec.SchemaNode, context: BuildContext) -> String {
      var html = "<p class=\"sk-openapi-schema-type\">Type: \(self.typeLabel(node, context: context))</p>"
      ...
      if let composition = node.composition {
         html += self.compositionHTML(composition, context: context)
      }

      if !node.properties.isEmpty {
         html += "<section class=\"sk-openapi-schema-props\"><h2>Properties</h2>\(self.propertyTable(node, context: context))</section>"
      }

      if node.type == "array", let item = node.items.first {
         html += "<section class=\"sk-openapi-array-items\"><h2>Items</h2><p>\(self.typeLabel(item, context: context))</p></section>"
      }
   ...
   static func compositionHTML(_ composition: OpenAPISpec.Composition, context: BuildContext) -> String {
      let members = composition.subschemas.map { "<li>\(self.typeLabel($0, context: context))</li>" }.joined()
