...
extension OpenAPISpec {
   ...
   public struct Composition: Sendable, Equatable {
      ...
      /// The `discriminator` of a polymorphic `oneOf` / `anyOf`: the property whose
      /// value selects the concrete variant, plus any explicit value→schema mapping.
      public struct Discriminator: Sendable, Equatable {
         /// The name of the property that selects the variant (for example `petType`).
         public let propertyName: String

         /// Explicit mappings from a discriminator value to a schema name, if declared.
      ...
      /// The discriminator selecting the variant, for a polymorphic `oneOf` / `anyOf`.
      public let discriminator: Discriminator?

      /// Memberwise initializer.
      public init(kind: Kind, subschemas: [SchemaNode], discriminator: Discriminator? = nil) {
         self.kind = kind
