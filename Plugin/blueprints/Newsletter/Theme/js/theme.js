document.addEventListener('DOMContentLoaded', function () {
   ...
   // Subscription confirmation – swap signup for welcome on ?subscribed=true
   if (new URLSearchParams(window.location.search).get('subscribed') === 'true') {
      var signupEl = document.getElementById('newsletter-signup');
      var welcomeEl = document.getElementById('newsletter-welcome');
      if (signupEl && welcomeEl) {
   ...
   if (searchBtn) {
      ...
      function renderInstantResults(container, query) {
         ...
         if (matched.length > 0) {
            html += '<div class="sk-search-section">ISSUES</div>';
   ...
   // Newsletter signup – inject after blog articles (not on legal pages)
   var articleBody = document.querySelector('.sk-article-body');
   var isLegalPage = document.body.classList.contains('sk-page-legal') || /\/(privacy|imprint)\/?$/.test(window.location.pathname);
   if (articleBody && !isLegalPage) {
      var signup = document.createElement('div');
      signup.className = 'newsletter-signup newsletter-signup-after-post';
      signup.innerHTML = '<h2>Enjoyed this issue?</h2>' +
         '<p>Get the next one delivered straight to your inbox.</p>' +
         ...
         '</form>' +
         '<small style="display: block; margin-top: 0.75rem; color: var(--color-text-secondary); font-size: 0.8rem;">No spam, unsubscribe anytime.</small>';
      articleBody.parentNode.insertBefore(signup, articleBody.nextSibling);
   }
});
