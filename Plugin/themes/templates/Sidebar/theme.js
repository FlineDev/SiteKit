document.addEventListener('DOMContentLoaded', function () {
   ...
   var sidebar = document.querySelector('.sk-site-header');
   if (sidebar) {
      // Create hamburger button
      var hamburger = document.createElement('button');
      hamburger.className = 'sk-sidebar-toggle';
      ...
      overlay.className = 'sk-sidebar-overlay';
      document.body.appendChild(overlay);

      function openSidebar() {
         sidebar.classList.add('sk-sidebar-open');
         overlay.classList.add('sk-sidebar-visible');
      ...
      function closeSidebar() {
         sidebar.classList.remove('sk-sidebar-open');
         overlay.classList.remove('sk-sidebar-visible');
         hamburger.setAttribute('aria-expanded', 'false');
         hamburger.innerHTML = '\u2630';
      ...
      hamburger.addEventListener('click', function () {
         var isOpen = sidebar.classList.contains('sk-sidebar-open');
         if (isOpen) {
            closeSidebar();
         } else {
            openSidebar();
      ...
      document.addEventListener('keydown', function (e) {
         if (e.key === 'Escape' && sidebar.classList.contains('sk-sidebar-open')) {
            closeSidebar();
         }
      });
   }

   // Table of Contents generation for article pages
   var articleBody = document.querySelector('.sk-article-body');
   var articleMain = document.querySelector('.sk-page-article .sk-main');
   if (articleBody && articleMain) {
      var headings = articleBody.querySelectorAll('h2, h3');
      if (headings.length >= 3) {
         var tocNav = document.createElement('nav');
         tocNav.className = 'sk-toc';
         tocNav.setAttribute('aria-label', 'Table of contents');

         var tocTitle = document.createElement('div');
         tocTitle.className = 'sk-toc-title';
         tocTitle.textContent = 'On this page';
         tocNav.appendChild(tocTitle);

         var tocList = document.createElement('ul');
         tocList.className = 'sk-toc-list';
         ...
         articleMain.appendChild(tocNav);
            ...
            entries.forEach(function (entry) {
               if (entry.isIntersecting) {
                  tocLinks.forEach(function (link) {
                     link.classList.remove('sk-toc-active');
                  });
                  var activeLink = tocList.querySelector('a[data-heading-id="' + entry.target.id + '"]');
                  if (activeLink) {
                     activeLink.classList.add('sk-toc-active');
                  }
               }
            });
         }, {
